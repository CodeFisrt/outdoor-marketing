...
app.delete("/screens/:id", (req, res) => {
  const { id } = req.params;
  db.query(
    "DELETE FROM outdoormarketingscreens WHERE ScreenID = ?",
    [id],
    (err) => {
      if (err) return res.status(500).send(err);
...
app.post("/hoardings", (req, res) => {
  ...
  // Fixed SQL with VALUES placeholders
  const sql = `
  ...
  // Make sure featured is numeric 1/0
  const values = [
    data.h_name,
    data.address,
    data.city,
...
app.put("/hoardings/:id", (req, res) => {
  ...
  const values = [
    ...
    data.contract_end_date,
    data.notes,
    data.featured ? 1 : 0, // <-- convert boolean to 1/0
    id,
  ];
